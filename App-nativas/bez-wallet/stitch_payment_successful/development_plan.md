...
> Creado: 2026-03-18 | Actualizado: 2026-03-23

---

## Resumen Ejecutivo

Las Fases 1 (Arquitectura), 2 (Smart Contracts × 16 sectores), 3 (Infraestructura), 4 (API Real), 4B (Wallet System), 5 (Frontend), 6 (Aegis AI), 7 (E2E Testing), 8 (DevOps), 9 (Blockchain Deployment) y 10 (Sistema de Validación) están **completadas**.
...
**Estado actual:** Todos los contratos compilados y testeados. Fases 3-11 completadas. Sistema de validación completamente integrado con API (12 endpoints), Frontend (6 páginas + widgets), Edge Node (heartbeat + webhooks + DePIN rewards), y Aegis AI (ValidatorMonitor + SlashingManager + 2 MCP tools + Grafana dashboard). Siguiente: Fase 12 (SDK de Validación y Onboarding B2B), Fase 13 (despliegue testnet/mainnet).
...
→ ✅ DevOps/Prod    → ✅ Deploy L2     → ✅ Validación     → 🔲 Integración   → 🔲 Testnet      → 🔲 Mainnet
     FASE 8              FASE 9            FASE 10            FASE 11            FASE 12           FASE 13
```
...
## FASE 12: SDK de Validación y Onboarding B2B 🔲 PENDIENTE
...
| # | Tarea | Archivo | Prioridad |
|---|-------|---------|-----------|
| 12A.1 | Clase `ValidatorClient` — register, addStake, heartbeat, claimRewards, getInfo | `sdk/modules/ValidatorClient.js` (nuevo) | CRÍTICA |
| 12A.2 | Clase `GovernanceClient` — propose, vote, delegate, getProposals | `sdk/modules/GovernanceClient.js` (nuevo) | ALTA |
| 12A.3 | ABIs de contratos de validación en `sdk/abis/` | `sdk/abis/ValidatorRegistry.json`, etc. | CRÍTICA |
| 12A.4 | Exportar desde `sdk/index.js` (ValidatorClient, GovernanceClient) | `sdk/index.js` (update) | ALTA |
| 12A.5 | Script CLI `register-validator.js` — registro interactivo desde terminal | `scripts/register-validator.js` | ALTA |
| 12A.6 | Script CLI `validator-status.js` — ver status del validador en terminal | `scripts/validator-status.js` | MEDIA |

### 12B — Onboarding Flow

| # | Tarea | Detalle | Prioridad |
|---|-------|---------|-----------|
| 12B.1 | Página `/onboarding` — wizard paso a paso para empresas nuevas | Frontend | ALTA |
| 12B.2 | Paso 1: Crear wallet empresarial (SmartWalletFactory) | Frontend + SDK | ALTA |
| 12B.3 | Paso 2: Adquirir BEZ (bridge o compra directa) | Frontend + Bridge UI | ALTA |
| 12B.4 | Paso 3: Registrar como validador (approve + register) | Frontend + SDK | ALTA |
| 12B.5 | Paso 4: Instalar Edge Node (instrucciones + descarga Docker) | Frontend (guide) | MEDIA |
| 12B.6 | Paso 5: Configurar ERP webhook (templates para SAP/Oracle/Shopify) | Docs + templates | MEDIA |
| 12B.7 | Email/notificación cuando el nodo empieza a validar | API notification service | BAJA |

### 12C — Templates de Integración ERP

| # | Template | Archivo | Prioridad |
|---|---------|---------|-----------|
| 12C.1 | SAP S/4HANA — SuiteScript para webhook | `docs/erp-templates/sap-webhook.md` | ALTA |
| 12C.2 | Oracle NetSuite — RESTlet para webhook | `docs/erp-templates/oracle-webhook.md` | ALTA |
| 12C.3 | Shopify — Webhook nativo config | `docs/erp-templates/shopify-webhook.md` | ALTA |
| 12C.4 | Generic HTTP — curl examples | `docs/erp-templates/generic-webhook.md` | MEDIA |

---

## FASE 13: Testnet Deployment 🔲 PENDIENTE
> **Objetivo:** Deploy de todos los contratos en Sepolia L2 real (no Anvil local)
> **Dependencia:** Fase 11 completada + auditoría de contratos
> **Prioridad:** ALTA
...
| # | Tarea | Detalle | Prioridad |
|---|-------|---------|-----------|
| 13A.1 | Auditoría Slither de los 7 contratos de validación | `scripts/security-audit.sh` (update) | CRÍTICA |
| 13A.2 | Auditoría Mythril (symbolic execution) | Nuevo script | ALTA |
| 13A.3 | Gas optimization review: validar que funciones no excedan block gas limit | forge test --gas-report | ALTA |
| 13A.4 | Fuzz testing con Foundry: invariant tests para ValidatorRegistry, SlashingManager | `test/core/invariant/` (nuevo) | ALTA |

### 13B — Deploy Testnet

| # | Tarea | Detalle | Prioridad |
|---|-------|---------|-----------|
| 13B.1 | Deploy contratos core + validación en Sepolia L2 | `forge script DeployAll --rpc-url $SEPOLIA_RPC --broadcast --verify` | CRÍTICA |
| 13B.2 | Verificar contratos en Blockscout explorer | Post-deploy verification | ALTA |
| 13B.3 | Registrar 3-5 validadores de prueba (diferentes tiers) | Script de seed | ALTA |
| 13B.4 | Simular epoch completo de sequencer rotation | Test end-to-end en testnet | ALTA |
| 13B.5 | Simular slashing + appeal + reversal | Test end-to-end en testnet | MEDIA |
| 13B.6 | Publicar deployment addresses en `deployments/{chainId}.json` | Post-deploy | ALTA |

### 13C — Testnet Monitoring

| # | Tarea | Detalle | Prioridad |
|---|-------|---------|-----------|
| 13C.1 | Configurar Prometheus para métricas de validadores on-chain | Prometheus exporter | ALTA |
| 13C.2 | Dashboard Grafana: validadores activos, stake total, epochs, slashes | `monitoring/grafana/dashboards/validators.json` | ALTA |
| 13C.3 | Alertas: validador inactivo >2h, slash ejecutado, sequencer fallo | Alertmanager rules | MEDIA |
...
DESPUÉS:
  ...
  FASE 12B → Onboarding wizard para empresas nuevas
