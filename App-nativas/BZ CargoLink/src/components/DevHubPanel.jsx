...
export default function DevHubPanel({ sector = 'wallet', onClose }) {
  ...
  const downloadSnippet = (label, cmd) => {
    const blob = new Blob([`${label}\n${cmd}\n`], { type: 'text/plain' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `bezhas-${sector}-${label.toLowerCase().replace(/[^a-z0-9]+/g, '-')}.txt`
    link.click()
    URL.revokeObjectURL(url)
    setNotice(`${label} preparado para descarga.`)
  }
        ...
        <div style={{ overflowY: 'auto', padding: 24, flex: 1 }}>
          ...
          {tab === 'api' && (
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', background: '#0e0e0e', border: '1px solid #222', borderRadius: 10, overflow: 'hidden', marginBottom: 20 }}>
                <input
                  readOnly value={platformState.apiKey}
                  type="password"
                ...
                <div style={{ padding: '0 12px', borderLeft: '1px solid #222' }}>
                  <CopyButton value={platformState.apiKey} />
                </div>
              </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
                ...
                <button
                  onClick={async () => {
                    try {
                      const selected = events.slice(0, bezhasPlatform.plans[platformState.planId].webhookLimit)
                      await cargoGateway.registerWebhook({ url: webhookUrl, events: selected }, platformState.apiKey)
                      bezhasPlatform.registerWebhook(webhookUrl, selected)
                      setNotice('Webhook guardado en el Core BeZhas.')
                    } catch (error) {
                      setNotice(error.message)
                    }
                  }}
                  style={{ padding: '12px 16px', background: '#00f0ff', color: '#000', borderRadius: 10, fontWeight: 900, fontSize: 11, border: 'none', cursor: 'pointer', flexShrink: 0 }}
                >
                  SAVE
                </button>
              </div>
              {notice && <p style={{ fontSize: 11, color: '#00f0ff', marginBottom: 12 }}>{notice}</p>}
                  ...
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderBottom: i < events.length - 1 ? '1px solid #111' : 'none' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <button
                        onClick={async () => {
                          try {
                            const targetUrl = webhookUrl || 'https://api.example.com/webhooks/bezhas'
                            await cargoGateway.registerWebhook({ url: targetUrl, events: [ev] }, platformState.apiKey)
                            bezhasPlatform.registerWebhook(targetUrl, [ev])
                            setNotice(`${ev} suscrito en el Core BeZhas.`)
                          } catch (error) {
                            setNotice(error.message)
                          }
                        }}
                        style={{ fontSize: 9, padding: '3px 8px', border: '1px solid #222', borderRadius: 12, color: '#555', cursor: 'pointer', background: 'transparent' }}
                      >
                        SUBSCRIBE
                      </button>
                    </div>
          ...
          {tab === 'sdk' && (
            <div>
                  ...
                  <div key={item.lang} style={{ background: '#0e0e0e', border: '1px solid #1a1a1a', borderRadius: 10, overflow: 'hidden' }}>
                    <div style={{ padding: '8px 14px', borderBottom: '1px solid #111', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        <button
                          onClick={() => downloadSnippet(item.lang, item.cmd)}
                          style={{ fontSize: 9, padding: '3px 8px', border: `1px solid ${item.color}30`, borderRadius: 8, color: item.color, cursor: 'pointer', background: 'transparent' }}
                        >
              ...
              {/* Full Docs */}
              <a href="/integration" style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 20, padding: '12px 16px', background: 'rgba(0,240,255,0.05)', border: '1px solid rgba(0,240,255,0.15)', borderRadius: 10, textDecoration: 'none', color: '#00f0ff', fontSize: 12, fontWeight: 800 }}>
                <Globe size={14} />
