...
#     que va DIRECTA a Secret Manager (${SECRET_PREFIX}DATABASE_URL): no se imprime nunca.
#
# La instancia se comparte con la versión anterior de la plataforma: aquí
# solo se añaden la base de datos y el usuario de BeZhas Blockchain.
#
# Las migraciones las aplica cada despliegue (Cloud Run Job `bezhas-db-migrate`),
...
echo "→ Usuario de aplicación y secreto ${SECRET_PREFIX}DATABASE_URL"
SECRETO_DB="${SECRET_PREFIX}DATABASE_URL"
existe gcloud secrets describe "$SECRETO_DB" || \
  gcloud secrets create "$SECRETO_DB" --replication-policy=automatic --labels=app=bezhas,platform=blockchain >/dev/null
TIENE_VERSION=$(gcloud secrets versions list "$SECRETO_DB" --filter=state=ENABLED --limit=1 --format='value(name)')
if [[ -z "$TIENE_VERSION" || "${ROTATE_DB_PASSWORD:-0}" == 1 ]]; then
  ...
  printf 'postgresql://%s:%s@%s:5432/%s' "$SQL_USER" "$CLAVE" "$IP_PRIVADA" "$SQL_DATABASE" \
    | gcloud secrets versions add "$SECRETO_DB" --data-file=- >/dev/null
  unset CLAVE
  echo "   Contraseña generada y guardada en Secret Manager (no se muestra)."
else
  echo "   $SECRETO_DB ya existe; no se toca (ROTATE_DB_PASSWORD=1 para rotarla)."
fi
gcloud secrets add-iam-policy-binding "$SECRETO_DB" --quiet \
...
✅ PostgreSQL listo: ${SQL_INSTANCE} (${IP_PRIVADA}, solo red privada).
   ${SECRET_PREFIX}DATABASE_URL está en Secret Manager; NO lo pongas en tu .env.
   Las migraciones se aplican solas en cada ./deploy/gcp/deploy.sh.

Siguiente paso: ./deploy/gcp/02-secrets.sh ~/bezhas-blockchain.env
MSG
