...
echo "Servicios"
declare -A SVC_OK=()
for svc in $(printf '%s\n' "$FRONTEND_SERVICE" "$BACKEND_SERVICE" "$MCP_SERVICE" \
               "$AEGIS_SERVICE" "$AIGW_SERVICE" "$AGENT_SERVICE" "$EDGE_SERVICE" | sort -u); do
...
if gcloud run jobs describe bezhas-db-migrate --region="$REGION" --project="$PROJECT_ID" >/dev/null 2>&1; then
  ult=$(gcloud run jobs executions list --job=bezhas-db-migrate --region="$REGION" --project="$PROJECT_ID" \
          --limit=1 --format='value(status.conditions[0].status)' 2>/dev/null)
  [[ "$ult" == True ]] && ok "migraciones aplicadas (bezhas-db-migrate)" \
    || mal "la última migración falló: gcloud run jobs executions list --job=bezhas-db-migrate --region=$REGION"
else
...
comprobar "$WWW_HOST" / 200 "$FRONTEND_SERVICE"
comprobar "$API_HOST" /api/health 200 "$BACKEND_SERVICE"
comprobar "$MCP_HOST" /.well-known/oauth-authorization-server 200 "$MCP_SERVICE"
...
# Ni los públicos (solo por el balanceador) ni los privados (solo con token
# OIDC) deben responder a un anónimo que llegue directo a *.run.app.
for svc in $(printf '%s\n' "$FRONTEND_SERVICE" "$BACKEND_SERVICE" "$AEGIS_SERVICE" \
               "$AIGW_SERVICE" "$AGENT_SERVICE" "$EDGE_SERVICE" | sort -u); do
  [[ -n "${SVC_OK[$svc]:-}" ]] || continue
  url=$(gcloud run services describe "$svc" --region="$REGION" --project="$PROJECT_ID" --format='value(status.url)' 2>/dev/null)
  [[ -n "$url" ]] || continue
  c=$(curl -s -o /dev/null -w '%{http_code}' --max-time 15 "$url/")
  [[ "$c" == 404 || "$c" == 403 ]] && ok "$svc: *.run.app cerrado a anónimos ($c)" \
                                   || mal "$svc: $url responde $c a un anónimo (ingress o IAM abiertos)"
done
