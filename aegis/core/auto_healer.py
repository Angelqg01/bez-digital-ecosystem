...
class AutoHealer:
    ...
    def get_healing_action(self, anomaly_type: str) -> str:
        ...
        action_map = {
            ...
            'validator_inactive': 'alert_validator_down',
            'heartbeat_expired': 'alert_validator_down',
            'heartbeat_stale': 'alert_validator_stale',
            'heartbeat_never_seen': 'alert_validator_down',
            'low_uptime': 'alert_validator_degraded',
            'stake_below_tier_minimum': 'alert_validator_degraded',
        }
    ...
    async def execute_action(self, action: str, context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            elif action in ('alert_validator_down', 'alert_validator_stale', 'alert_validator_degraded'):
                return await self.handle_validator_alert(action, context)
    ...
    async def handle_validator_alert(self, action: str, context: Dict[str, Any]) -> bool:
        """
        Handle validator-related alerts: downtime, stale heartbeat, degraded performance.
        In production, sends notification and optionally triggers slashing check.
        """
        operator = context.get('operator', context.get('address', 'unknown'))
        severity = 'critical' if action == 'alert_validator_down' else 'warning'
        logger.warning(f"🛡️ Validator alert [{severity}]: {action} for {operator}")

        # Log alert persistently
        try:
            ...
            logger.error(f"Failed to store validator alert: {e}")

        # In production: push to notification queue (Redis pub/sub)
        try:
            await self.redis.set_rate_limit(
                f"validator_alert:{operator}", ttl=3600
            )
        except Exception:
            pass

        logger.info(f"✅ Validator alert logged for {operator}")
        return True
